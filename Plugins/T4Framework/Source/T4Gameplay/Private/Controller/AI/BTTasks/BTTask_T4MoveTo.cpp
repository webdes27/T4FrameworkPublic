...
#include "Classes/Controller/AI/BTTasks/BTTask_T4MoveTo.h"
...
)
{
	...
	if (nullptr != NPCController)
	{
		const FT4NPCAIMemory& AIMemory = NPCController->GetAIMemoryConst(); // #50
		...
		const FVector TargetLocation = AIMemory.MoveTargetLocation;
