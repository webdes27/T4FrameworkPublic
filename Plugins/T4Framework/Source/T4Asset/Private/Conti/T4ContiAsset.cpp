...
FT4ContiActionStruct* FT4ActionCompositeData::NewAndAddAction(
	ET4ActionType InNewActionType
)
{
	// #24
	// #T4_ADD_ACTION_TAG_CONTI
	FT4ContiActionStruct* ReturnAction = nullptr;
	...
	switch (InNewActionType)
	{
		case ET4ActionType::Branch: // #54
			{
				...
				ReturnAction = static_cast<FT4ContiActionStruct*>(&NewAction);
			}
			break;
		...
		case ET4ActionType::LayerSet: // #81
			{
				FT4LayerSetAction& NewAction = LayerSetActions.AddDefaulted_GetRef();
				AllocedHeaderKey = GetNewHeaderKey();
				ChildActionArrayIndex = LayerSetActions.Num() - 1;
				ReturnAction = static_cast<FT4ContiActionStruct*>(&NewAction);
			}
			break;
		...
		default:
			{
				UE_LOG(
					LogT4Asset,
					Error,
					TEXT("NewAndAddAction : Unknown action type '%u'"),
...
FT4ContiActionStruct* FT4ActionCompositeData::CloneAndAddAction(
	uint32 InSourceHeaderKey
) // #65
{
	FT4ContiActionStruct* SourceActionBase = GetActionStruct(InSourceHeaderKey);
	if (nullptr == SourceActionBase)
	{
		return nullptr;
	}
	// #T4_ADD_ACTION_TAG_CONTI
	FT4ContiActionStruct* ReturnActionBase = nullptr;
	switch (SourceActionBase->ActionType)
	{
		case ET4ActionType::Branch: // #54
			{
				...
				ReturnActionBase = static_cast<FT4ContiActionStruct*>(TargetAction);
			}
			break;
		...
		case ET4ActionType::LayerSet: // #81
			{
				FT4LayerSetAction* TargetAction = AddChild<FT4LayerSetAction>();
				check(nullptr != TargetAction);
				CopyAction<FT4LayerSetAction>(SourceActionBase, TargetAction);
				ReturnActionBase = static_cast<FT4ContiActionStruct*>(TargetAction);
			}
			break;
		...
		default:
			{
				UE_LOG(
					LogT4Asset,
					Error,
					TEXT("CloneAndAddAction : Unknown action type '%u'"),
...
FT4ContiActionStruct* FT4ActionCompositeData::GetActionStruct(
...
) // #56
{
	...
	switch (HierarchyInfo.ActionType)
	{
		case ET4ActionType::Branch: // #54
			{
				...
				return static_cast<FT4ContiActionStruct*>(&CurrentAction);
			}
			break;
		...
		case ET4ActionType::LayerSet: // #81
			{
				check(HierarchyInfo.ActionArrayIndex < LayerSetActions.Num());
				FT4LayerSetAction& CurrentAction = LayerSetActions[HierarchyInfo.ActionArrayIndex];
				return static_cast<FT4ContiActionStruct*>(&CurrentAction);
			}
			break;
		...
		default:
			{
				UE_LOG(
					LogT4Asset,
					Error,
					TEXT("GetActionStruct : Unknown action type '%u'"),
...
static const float ContiMaxPlayTimeSec = 10.0f; // #56

UT4ContiAsset::UT4ContiAsset(const FObjectInitializer& ObjectInitializer)
	: Super(ObjectInitializer)
	, TotalPlayTimeSec(ContiMaxPlayTimeSec) // #56
...
void UT4ContiAsset::PostLoad()
{
	...
	if (ContiVar < FT4ContiCustomVersion::CommonPropertyNameChanged)
	{
		...
		{
			FT4ContiActionStruct* ContiActionStruct = CompositeData.GetActionStruct(It.Key());
