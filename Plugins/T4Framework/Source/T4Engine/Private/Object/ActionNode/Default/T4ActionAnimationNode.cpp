...
#include "Public/Action/T4ActionContiCommands.h"
...
/**
  *
 */
FT4ActionAnimationNode::FT4ActionAnimationNode(FT4ActionNodeControl* InControl, const FT4ActionKey& InKey)
...
bool FT4ActionAnimationNode::Create(const FT4ActionCommand* InAction)
{
	check(ET4ActionType::Animation == InAction->ActionType);
	const FT4AnimationAction& ConvAction = *(static_cast<const FT4AnimationAction*>(InAction));
	AnimParameters.AnimMontageName = T4Const_SkillAnimMontageName;
...
void FT4ActionAnimationNode::Stop()
{
	...
	if (OwnerGameObject->IsLoaded())
	{
		...
		AnimControl->StopAnimation(PlayAnimInstanceID, T4Const_DefaultAnimBlendTimeSec);
	}
	PlayAnimInstanceID = INDEX_NONE;
}
...
bool FT4ActionAnimationNode::PlayInternal(float InOffsetTimeSec)
{
	...
	if (AnimParameters.SectionName == NAME_None)
	{
		T4_LOG(
			Warning,
			TEXT("AnimSequence SectionName is empty.")
		);
		OnStop();
		return false;
	}
	...
	PlayAnimInstanceID = OwnerGameObject->PlayAnimationAndBroadcast(AnimParameters);
	if (INDEX_NONE == PlayAnimInstanceID)
	{
		OnStop();
		return false;
