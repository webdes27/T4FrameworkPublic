...
/**
  * #101
 */
FT4ActionCameraShakeNode::FT4ActionCameraShakeNode(FT4ActionNodeControl* InControl, const FT4ActionKey& InKey)
...
bool FT4ActionCameraShakeNode::Create(const FT4ActionCommand* InAction)
{
	check(ET4ActionType::CameraShake == InAction->ActionType);
...
bool FT4ActionCameraShakeNode::PlayInternal(float InOffsetTimeSec)
{
	...
	CameraShakePtr->PlayShake(
		Cast<APlayerCameraManager>(CameraManager), 
		PlayScale, 
		PlaySpace, 
		UserDefinedPlaySpace
	);
