...
/**
  *
 */
FT4ActionNodeBaseGraph::FT4ActionNodeBaseGraph()
{
}
...
void FT4ActionNodeBaseGraph::Reset()
{
#if 0
	for (FT4ActionNodeBase* ActionNode : DestoryBeforeFrameNodes)
	{
	...
	for (FT4ActionNodeBase* ActionNode : ChildNodes)
	{
...
void FT4ActionNodeBaseGraph::Advance(const FT4UpdateTime& InUpdateTime)
{
...
void FT4ActionNodeBaseGraph::AddChildNode(
	const FT4ActionKey& InActionKey, 
	FT4ActionNodeBase* InNewNode
)
{
...
bool FT4ActionNodeBaseGraph::RemoveChildNode(
	const FT4ActionKey& InActionKey,
...
)
{
	...
	TArray<FT4ActionNodeBase*> FoundActionRootNodes;
	ChildMultiMap.MultiFind(InActionKey, FoundActionRootNodes);
	for (FT4ActionNodeBase* ActionNode : FoundActionRootNodes)
	{
		check(nullptr != ActionNode);
...
IT4ActionNode* FT4ActionNodeBaseGraph::GetChilldNodeByPrimary(
...
bool FT4ActionNodeBaseGraph::GetChildNodes(
...
bool FT4ActionNodeBaseGraph::IsPlaying() const
{
	for (FT4ActionNodeBase* ActionNode : ChildNodes)
	{
		check(nullptr != ActionNode);
...
bool FT4ActionNodeBaseGraph::IsLooping() const
{
	for (FT4ActionNodeBase* ActionNode : ChildNodes)
	{
		check(nullptr != ActionNode);
...
void FT4ActionNodeBaseGraph::OnStopAllChildren() // #63
{
	for (FT4ActionNodeBase* ActionNode : ChildNodes)
	{
		check(nullptr != ActionNode);
...
bool FT4ActionNodeBaseGraph::CheckFinished() const // #56 : 하위 노드까지 검사하는 처리 추가
{
	for (FT4ActionNodeBase* ActionNode : ChildNodes)
	{
		check(nullptr != ActionNode);
...
bool FT4ActionNodeBaseGraph::CheckStopedAndFinished() const
{
	for (FT4ActionNodeBase* ActionNode : ChildNodes)
	{
		check(nullptr != ActionNode);
...
uint32 FT4ActionNodeBaseGraph::NumChildActions() const
{
...
bool FT4ActionNodeBaseGraph::IsDebugPaused() const
{
	for (FT4ActionNodeBase* ActionNode : ChildNodes)
	{
		check(nullptr != ActionNode);
...
void FT4ActionNodeBaseGraph::SetDebugPause(bool bPause) // #54
{
	for (FT4ActionNodeBase* ActionNode : ChildNodes)
	{
		check(nullptr != ActionNode);
