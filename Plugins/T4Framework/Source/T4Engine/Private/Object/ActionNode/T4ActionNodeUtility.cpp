...
float FT4ActionEasingCurveBlender::GetBlendWeight(float InPlayingTimeSec, float InPlayTimeLeft) const // #100, #58, #102 : 0 ~ 1
{
	...
	float EvaluateWeight = T4EngineEnvironment::Evaluate(
		EasingType,
		CurrentWeight
	);
...
namespace T4ActionNodeUtility
{
	...
	) // #101 : PlayTarget 옵션에 따라 스스로 처리할지 Player 에게 전달할지를 결정!
	{
		...
		const bool bPlayer = OwnerGameObject->HasPlayer();
		...
		else
		{
			...
			PlayerObject->OnExecuteAction(InAction, InParameters); // 플레이어에게 던진다.
			return false;
		}
		return true;
	}
