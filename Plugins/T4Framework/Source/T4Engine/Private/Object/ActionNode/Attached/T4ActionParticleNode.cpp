...
/**
  *
 */
FT4ActionParticleNode::FT4ActionParticleNode(FT4ActionNodeControl* InControl, const FT4ActionKey& InKey)
...
bool FT4ActionParticleNode::Create(const FT4ActionCommand* InAction)
{
	check(ET4ActionType::Particle == InAction->ActionType);
...
bool FT4ActionParticleNode::Play()
{
	...
	ParticleSystemComponentPtr = NewComponentTemplate<UT4ParticleSystemComponent>(OwnerGameObject->GetPawn(), true);
	{
	...
	else if (ET4LoadingPolicy::Async == LoadingPolicy)
	{
		...
		else if (CheckLoadState(ALS_Failed))
		{
			T4_LOG(
				Error,
				TEXT("Failed to load ParticleSystem '%s'"),
				*(AssetPath.ToString())
			);
			OnStop();
			return false;
