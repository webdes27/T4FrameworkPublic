	...
	, CharacterOwner(nullptr)
{
	...
	bRequestedMoveUseAcceleration = false; // #109 : 가감속 사용하지 않음. 이후 관련 코드 일괄 삭제할 것!
...
void UT4MovementComponent::StartNewPhysics(float DeltaTime, int32 Iterations)
{
	...
	if (UpdatedComponent->IsSimulatingPhysics())
	{
		T4_LOG(
			Log, 
			TEXT("UpdateComponent (%s) is simulating physics - aborting."), 
...
)
{
	...
	if (NavMeshProjectionTimer <= 0.0f)
	{
		...
		else
		{
			T4_LOG(
				VeryVerbose, 
				TEXT("%s interval: %.3f velocity: %s [SKIP TRACE]"), 
				*GetNameSafe(CharacterOwner), 
				NavMeshProjectionInterval, 
				*Velocity.ToString()
			);
...
)
{
	...
	if (Hit.IsValidBlockingHit())
	{
		...
		{
			//T4_LOG(VeryVerbose, TEXT("- Reject StepUp (too high Height %.3f) up from floor base %f to %f"), DeltaZ, PawnInitialFloorBaseZ, NewLocation.Z);
			ScopedStepUpMovement.RevertMove();
			return false;
		}
		...
		if (!IsWalkable(Hit))
		{
			...
			if (bNormalTowardsMe)
			{
				//T4_LOG(VeryVerbose, TEXT("- Reject StepUp (unwalkable normal %s opposed to movement)"), *Hit.ImpactNormal.ToString());
				ScopedStepUpMovement.RevertMove();
				return false;
			}
			...
			{
				//T4_LOG(VeryVerbose, TEXT("- Reject StepUp (unwalkable normal %s above old position)"), *Hit.ImpactNormal.ToString());
				ScopedStepUpMovement.RevertMove();
				return false;
			}
		}
		...
		if (!IsWithinEdgeTolerance(Hit.Location, Hit.ImpactPoint, PawnRadius))
		{
			//T4_LOG(VeryVerbose, TEXT("- Reject StepUp (outside edge tolerance)"));
			ScopedStepUpMovement.RevertMove();
			return false;
		}
		...
		{
			//T4_LOG(VeryVerbose, TEXT("- Reject StepUp (up onto surface with !CanStepUp())"));
			ScopedStepUpMovement.RevertMove();
			return false;
		}
...
) const
{
	...
	if (FloorLineTraceDist > 0.f || FloorSweepTraceDist > 0.f)
	{
		...
		else
		{
			...
			{
				//T4_LOG(Log, TEXT("%s SKIP check for floor"), *CharacterOwner->GetName());
...
{
	T4_LOG(VeryVerbose, TEXT("[Role:%d] %s at location %s"), (int32)PawnOwner->GetLocalRole(), *GetNameSafe(PawnOwner), *CapsuleLocation.ToString());
	OutFloorResult.Clear();
...
void UT4MovementComponent::AdjustFloorHeight()
{
	...
	if (OldFloorDist < T4_MIN_FLOOR_DIST || OldFloorDist > T4_MAX_FLOOR_DIST)
	{
		...
		T4_LOG(VeryVerbose, TEXT("Adjust floor height %.3f (Hit = %d)"), MoveDist, AdjustHit.bBlockingHit);
...
{
	//T4_LOG(Log, TEXT("RevertMove from %f %f %f to %f %f %f"), CharacterOwner->Location.X, CharacterOwner->Location.Y, CharacterOwner->Location.Z, OldLocation.X, OldLocation.Y, OldLocation.Z);
	UpdatedComponent->SetWorldLocation(OldLocation, false, nullptr, GetTeleportType());

	//T4_LOG(Log, TEXT("Now at %f %f %f"), CharacterOwner->Location.X, CharacterOwner->Location.Y, CharacterOwner->Location.Z);
	bJustTeleported = false;
	...
	if (bFailMove)
	{
		...
		Velocity = FVector::ZeroVector;
		Acceleration = FVector::ZeroVector;
		//T4_LOG(Log, TEXT("%s FAILMOVE RevertMove"), *CharacterOwner->GetName());
	}
}
...
) const
{
	...
	switch (ShrinkMode)
	{
		...
		default:
			T4_LOG(
				Warning,
				TEXT("Unknown EShrinkCapsuleExtent")
			);
			break;
	}
	...
	if (RemainingTime > MaxSimulationTimeStep)
	{
		...
		else
		{
			...
			if ((s_WarningCount++ < 100) || (GFrameCounter & 15) == 0)
			{
				T4_LOG(
					Warning,
					TEXT("Max iterations %d hit while remaining time %.6f > MaxSimulationTimeStep (%.3f) for '%s', movement '%s'"),
