...
UCLASS()
class UT4EnvironmentZoneComponent : public USceneComponent
{
...
public:
	void Initialize(
		...
		float InOffsetTimeSec // #99 : 일반적으로 Env 는 Blending 때문에 OffsetTime 이 0이 되어야 하나, 타이밍을 맞추기 위한 처리도 필요할 것이다. Conti Play/Pause
	);
	void Reset();
...
public:
	...
	float InitializeOffsetTimeSec; // #99 : 일반적으로 Env 는 Blending 때문에 OffsetTime 이 0이 되어야 하나, 타이밍을 맞추기 위한 처리도 필요할 것이다. Conti Play/Pause
