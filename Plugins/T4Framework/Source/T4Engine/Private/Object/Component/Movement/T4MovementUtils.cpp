		...
		if (MovementBase)
		{
			if (BoneName != NAME_None)
			{
				...
				if (MovementBase)
				{
					...
					{
						UE_LOG(LogT4Engine, Warning, TEXT("GetMovementBaseTransform(): Invalid bone or socket '%s' for PrimitiveComponent base %s"), *BoneName.ToString(), *GetPathNameSafe(MovementBase));
					}
				}
