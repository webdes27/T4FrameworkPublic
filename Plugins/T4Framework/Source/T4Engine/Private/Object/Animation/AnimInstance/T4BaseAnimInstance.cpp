...
bool UT4BaseAnimInstance::HandleNotify(const FAnimNotifyEvent& InAnimNotifyEvent)
{
	...
	UE_LOG(
		LogT4Engine,
		Verbose,
		TEXT("HandleNotify : AnimNotify EventName '%s'"),
		*(InAnimNotifyEvent.NotifyName.ToString())
	);
#endif
	if (T4AnimSetAnimNotifyFootstepName != InAnimNotifyEvent.NotifyName)
	{
		return false;
	}
	...
	{
		OnAnimNotify.ExecuteIfBound(T4AnimSetAnimNotifyLeftFootstepName);
	}
	else
	{
		OnAnimNotify.ExecuteIfBound(T4AnimSetAnimNotifyRightFootstepName);
	}
	return true;
}
...
bool UT4BaseAnimInstance::IsValidSection(
...
)
{
	...
	else
	{
		UE_LOG(
			LogT4Engine,
			Error,
			TEXT("StopAnimation : Unknown anim montage type '%s'"),
...
)
{
	...
	else
	{
		UE_LOG(
			LogT4Engine,
			Error,
			TEXT("PlayAnimation : Unknown Anim Montage Type '%s'"),
