...
{
	RegisterAnimState(T4AnimStateEmptyName, new FT4EmptyAnimState());
	RegisterAnimState(T4AnimStateErrorName, new FT4ErrorAnimState());
	ActiveAnimStateName = T4AnimStateEmptyName;
}
...
)
{
	if (!AnimStates.Contains(InAnimStateName))
	{
		UE_LOG(
			LogT4Engine,
			Verbose,
			TEXT("TryChangeAnimState : AnimState '%s' is  Not found."),
		...
		if (nullptr != ActiveAnimState)
		{
			...
			if (ActivePriority < NewPriority)
			{
				UE_LOG(
					LogT4Engine,
					Verbose,
					TEXT("TryChangeAnimState : Unable to Change AnimState '%s(%u)'. Higher Priority ActiveAnimState '%s(%u)'."),
					*(InAnimStateName.ToString()),
					uint8(NewPriority),
		...
		if (nullptr != PendingAnimState)
		{
			...
			if (PendingPriority < NewPriority)
			{
				UE_LOG(
					LogT4Engine,
					Verbose,
					TEXT("TryChangeAnimState : Unable to Change AnimState '%s(%u)'. Higher Priority PendingAnimState '%s(%u)'."),
					*(InAnimStateName.ToString()),
					uint8(NewPriority),
...
)
{
	...
	if (AnimStates.Contains(InAnimStateName))
	{
		UE_LOG(
			LogT4Engine,
			Warning,
			TEXT("RegisterAnimState : Already Registered AnimState '%s'."),
			*(InAnimStateName.ToString())
		);
		return;
	}
...
)
{
	if (!AnimStates.Contains(InAnimStateName))
	{
		UE_LOG(
			LogT4Engine,
			Warning,
			TEXT("UnregisterAnimState : AnimState '%s' is  Not found."),
			*(InAnimStateName.ToString())
		);
		return;
	}
...
bool FT4BaseAnimControl::IsValidSection(
...
)
{
	...
	return AnimInstance->IsValidSection(InAnimMontageName, InSectionName);
}
		...
		default:
			{
				UE_LOG(
					LogT4Engine,
					Error,
					TEXT("[SL:%u] FT4BaseAnimControl::CreateNewControl : Unknown anim instance type '%u'"),
...
void FT4BaseAnimControl::ProcessPendingAnimState() // #47
{
	...
	if (nullptr != PendingAnimState)
	{
		...
		const ET4AnimStatePriority& PendingPriority = PendingAnimState->GetPriority();
		const ET4AnimStatePriority& NextPriority = (nullptr != NextAnimState) ? NextAnimState->GetPriority() : ET4AnimStatePriority::AnimPriority_Low;
		if (PendingPriority > NextPriority)
		{
			UE_LOG(
				LogT4Engine,
				Verbose,
				TEXT("ProcessPendingAnimState : Unable to Change AnimState '%s(%u)'. Pending AnimState '%s(%u)'."),
				*(PendingAnimState->GetName().ToString()),
				uint8(PendingPriority),
...
UT4BaseAnimInstance* FT4BaseAnimControl::GetAnimInstance()
{
	...
	return GameObject->GetAnimInstance();
}
