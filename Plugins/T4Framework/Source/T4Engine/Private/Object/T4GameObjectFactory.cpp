		...
		default:
			{
				UE_LOG(
					LogT4Engine,
					Error,
					TEXT("CreateCharacterObject : Unknown Actor mesh type '%u'"),
					uint8(EntityAsset->MeshType)
				);
			}
			break;
	}
		...
		default:
			{
				UE_LOG(
					LogT4Engine,
					Error,
					TEXT("CreatePropObject : Unknown prop type '%u'"),
...
)
{
		...
		default:
			{
				UE_LOG(
					LogT4Engine,
					Error,
					TEXT("[SL:%u] FT4GameObjectFactory : Unknown EntityAsset type '%u'"),
					uint8(InLayerType),
					uint8(InAction->EntityType)
				);
			}
			break;
	}
	...
	if (nullptr == NewGameObject)
	{
		UE_LOG(
			LogT4Engine,
			Error,
			TEXT("FT4GameObjectFactory : Failed to create or spawn. EntityAsset '%s'"),
...
) // #54
{
		...
		default:
			{
				UE_LOG(
					LogT4Engine,
					Error,
					TEXT("CreateWorldObject : Unknown WorldObject type '%u'"),
					uint8(InWorldObjectType)
				);
			}
			break;
	}
	...
	if (nullptr == NewGameObject)
	{
		UE_LOG(
			LogT4Engine,
			Error,
			TEXT("FT4GameObjectFactory : Failed to Create WorldObject.")
		);
		return nullptr;
	}
