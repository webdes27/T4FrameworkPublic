...
/**
  *
 */
FT4ActionContiNode::FT4ActionContiNode(FT4ActionControl* InControl, const FT4ActionKey& InKey)
...
)
{
	...
	if (InAction.ContiAsset.IsNull())
	{
		UE_LOG(
			LogT4Engine,
			Error,
			TEXT("FT4ActionContiNode::CreateNode Failed. Not set ContiAsset.")
		);
		return nullptr;
	}
...
bool FT4ActionContiNode::Create(const FT4ActionStruct* InAction)
{
	check(ET4ActionType::Conti == InAction->ActionType);
	...
	else if (ET4LoadingPolicy::Async == LoadingPolicy)
	{
		...
		else if (CheckLoadState(ALS_Failed))
		{
			UE_LOG(
				LogT4Engine,
				Error,
				TEXT("FT4ActionContiNode::Play Failed to load ContiAsset '%s'."),
				*(AssetPath.ToString())
			);
			OnStop();
			return false;
