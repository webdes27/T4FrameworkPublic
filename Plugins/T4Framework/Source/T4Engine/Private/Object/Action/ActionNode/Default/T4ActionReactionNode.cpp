...
/**
  *
 */
FT4ActionReactionNode::FT4ActionReactionNode(FT4ActionControl* InControl, const FT4ActionKey& InKey)
...
bool FT4ActionReactionNode::Create(const FT4ActionStruct* InAction)
{
	check(ET4ActionType::Reaction == InAction->ActionType);
...
bool FT4ActionReactionNode::Play()
{
	...
	if (!bResult)
	{
		UE_LOG(
			LogT4Engine,
			Error,
			TEXT("FT4ActionReactionNode::Play : Failed to Reaction '%s'"),
		...
		case ET4EntityReactionType::Die:
			{
				...
				OwnerObject->DoExecuteAction(&NewAction, nullptr);
			}
			break;
		...
		default:
			{
				UE_LOG(
					LogT4Engine,
					Error,
					TEXT("PFT4ActionReactionNode::PlayInternal '%u' failed. no implementation."),
					uint8(ReactionType)
				);
				return false;
			}
