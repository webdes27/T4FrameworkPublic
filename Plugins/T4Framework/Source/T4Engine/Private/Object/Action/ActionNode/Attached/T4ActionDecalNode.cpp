...
/**
  *
 */
FT4ActionDecalNode::FT4ActionDecalNode(FT4ActionControl* InControl, const FT4ActionKey& InKey)
...
bool FT4ActionDecalNode::Create(const FT4ActionStruct* InAction)
{
	check(ET4ActionType::Decal == InAction->ActionType);
...
bool FT4ActionDecalNode::Play()
{
	...
	DecalComponentPtr = NewComponentTemplate<UT4DecalComponent>(true);
	{
	...
	else if (ET4LoadingPolicy::Async == LoadingPolicy)
	{
		...
		else if (CheckLoadState(ALS_Failed))
		{
			UE_LOG(
				LogT4Engine,
				Error,
				TEXT("FT4ActionDecalNode::Play Failed to load DecalMaterial '%s'."),
				*(AssetPath.ToString())
			);
			OnStop();
			return false;
