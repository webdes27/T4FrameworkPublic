...
/**
  *
 */
FT4ActionBranchNode::FT4ActionBranchNode(FT4ActionControl* InControl, const FT4ActionKey& InKey)
	...
	if (ET4BranchCondition::CompareActiveName == InContition)
	{
		if (nullptr == InParameters || InConditionName == NAME_None)
		{
			UE_LOG(
				LogT4Engine,
				Verbose,
				TEXT("FT4ActionBranchNode::CheckCondition Failed. ActionParameters is not set.")
			);
			return false;
		}
		if (!InParameters->CheckBits(ET4DefaultParamBits::ContidionNameBit))
		{
			UE_LOG(
				LogT4Engine,
				Verbose,
				TEXT("FT4ActionBranchNode::CheckCondition Failed. must be set ActionParameters::SetConditionName.")
			);
			return false;
		}
		...
		if (InConditionName != DefaultParameters.ActiveConditionName)
		{
			UE_LOG(
				LogT4Engine,
				Verbose,
				TEXT("FT4ActionBranchNode::CheckCondition Failed. No Active ConditionName '%s'."),
...
)
{
	...
	if (InAction.ContiAsset.IsNull())
	{
		UE_LOG(
			LogT4Engine,
			Error,
			TEXT("FT4ActionBranchNode::CreateNode Failed. Not set ContiAsset.")
		);
		return nullptr;
	}
...
bool FT4ActionBranchNode::Create(const FT4ActionStruct* InAction)
{
	check(ET4ActionType::Branch == InAction->ActionType);
	...
	else if (ET4LoadingPolicy::Async == LoadingPolicy)
	{
		...
		else if (CheckLoadState(ALS_Failed))
		{
			UE_LOG(
				LogT4Engine,
				Error,
				TEXT("FT4ActionBranchNode::Play Failed to load ContiAsset '%s'."),
				*(AssetPath.ToString())
			);
			OnStop();
			return false;
