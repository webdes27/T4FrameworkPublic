...
/**
  *
 */
FT4ActionNode::FT4ActionNode(FT4ActionControl* InControl, const FT4ActionKey& InKey)
	: ActionKey(InKey)
...
FT4ActionNode::~FT4ActionNode()
{
}

bool FT4ActionNode::IsPlaying() const
{ 
	return (bDebugPaused) ? bDebugPaused : (APS_Playing == PlayState) ? true : false; 
}

template <class T, class U>
FORCEINLINE FT4ActionNode* CreateNodeTemplate(
...
)
{
	...
	{
		if (ET4ActionStructType::Code == InAction.GetActionStructType()) // #62
		{
			UE_LOG(
				LogT4Engine,
				Verbose,
				TEXT("CreateNodeTemplate : Code '%s' CreateNode Failed"),
				*InAction.ToString()
			);
		}
		else
		{
			UE_LOG(
				LogT4Engine,
				Verbose,
				TEXT("CreateNodeTemplate : Conti '%s' CreateNode Failed"),
				*InAction.ToString()
			);
		}
		return nullptr;
...
FT4ActionNode* FT4ActionNode::CreateNode(
	FT4ActionControl* InControl,
...
)
{
	...
	FT4ActionNode* NewChildAction = nullptr;
	switch (InActionType)
	{
		...
		case ET4ActionType::LayerSet: // #81
			NewChildAction = CreateNodeTemplate<FT4ActionLayerSetNode>(
				InControl,
				*(static_cast<const FT4LayerSetAction*>(InAction)),
				InParameters // 
				);
			break;
		...
		default:
			{
				UE_LOG(
					LogT4Engine,
					Error,
					TEXT("FT4ActionNode::CreateNode '%s' failed. no implementation."),
					*(InAction->ToString())
				);
			}
			break;
...
void FT4ActionNode::DestroyNode(FT4ActionNode** InActionNode)
{
...
IT4ActionNode* FT4ActionNode::AddChildNode(
	const FT4ActionStruct* InAction,
...
)
{
	...
	if (ET4LifecycleType::Duration == InAction->LifecycleType)
	{
		...
		if ((InAction->StartTimeSec + InAction->DurationSec) <= InOffsetTimeSec)
		{
			UE_LOG(
				LogT4Engine,
				VeryVerbose,
				TEXT("FT4ActionNode::AddChildNode '%s' skiped. (DurationSec <= InOffsetTimeSec)"),
				*(InAction->ToString())
			);
			return nullptr;
		}
	}
	FT4ActionNode* NewChildAction = FT4ActionNode::CreateNode(
		ActionControlRef, 
		InAction,
	...
	{
		FT4ActionNode::DestroyNode(&NewChildAction);
		return nullptr;
	}
	return static_cast<IT4ActionNode*>(NewChildAction);
}

bool FT4ActionNode::AddChildNodeInternal(
	const FT4ActionStruct* InAction,
	FT4ActionNode* InActionNode
)
{
	...
	else
	{
		UE_LOG(
			LogT4Engine,
			VeryVerbose,
			TEXT("FT4ActionNode::AddChildNode '%s' OnCreate failed."),
			*(InAction->ToString())
		);
	}
	return bResult;
...
bool FT4ActionNode::RemoveChildNode(const FT4StopAction* InAction)
{
...
uint32 FT4ActionNode::NumChildActions() const
{
	return ActionNodeGraph.NumChildActions();
}

bool FT4ActionNode::OnCreate(const FT4ActionStruct* InAction)
{
	bool bResult = Create(InAction);
	return bResult;
}

void FT4ActionNode::OnDestroy()
{
...
void FT4ActionNode::OnDestroying() // #102
{
	Destroying();
}

void FT4ActionNode::OnAdvance(const FT4UpdateTime& InUpdateTime)
{
...
void FT4ActionNode::OnStartLoading()
{
...
bool FT4ActionNode::IsDestroyable() const
{
	bool bIsFinished = CheckStopedAndFinished();
	return bIsFinished;
}

bool FT4ActionNode::CheckFinished() const
{
		...
		default:
			{
				UE_LOG(
					LogT4Engine,
					Error,
					TEXT("CheckFinished : Unknown LifecycleType '%u'"),
					uint8(LifecycleType)
				);
			}
			break;
...
bool FT4ActionNode::CheckStopedAndFinished() const
{
...
float FT4ActionNode::GetGlobalMaxPlayTimeSec() const
{
	if (ActionParameterPtr.IsValid())
	{
		// #58 : 코드 호출 또는 툴에서 최대 시간 제한에 사용
		if (ActionParameterPtr->CheckBits(ET4TimeParamBits::OverrideMaxPlayTimeBit))
		{
			return ActionParameterPtr->GetTimeParams().OverrideMaxPlayTimeSec;
		}
	}
	return GetContiMaxPlayTimeSec();
}

void FT4ActionNode::SetActionParameters(
...
void FT4ActionNode::SetLifecycleType(
...
AT4GameObject* FT4ActionNode::GetGameObject() const
{
	check(nullptr != ActionControlRef);
	return ActionControlRef->GetGameObject();
}

IT4GameWorld* FT4ActionNode::GetGameWorld() const
{
	return T4EngineWorldGet(GetGameObject()->GetLayerType());
}

AT4GameObject* FT4ActionNode::NewClientObject(
...
) // #54
{
	...
	IT4GameObject* NewWorldObject = GameWorld->GetContainer()->CreateClientObject(
		InWorldObjectType,
		InName,
		InLocation,
		InRotation,
...
void FT4ActionNode::DeleteClientObject() // #54
{
	...
	IT4GameWorld* GameWorld = GetGameWorld();
	check(nullptr != GameWorld);
	GameWorld->GetContainer()->DestroyClientObject(WorldObjectID);
	WorldObjectID.SetNone();
}

AT4GameObject* FT4ActionNode::GetClientObject() const
{
	if (!WorldObjectID.IsValid())
	{
		return nullptr;
	...
	IT4GameWorld* GameWorld = GetGameWorld();
	check(nullptr != GameWorld);
	IT4GameObject* WorldObject = GameWorld->GetContainer()->FindGameObject(WorldObjectID);
...
FT4ActionKey FT4ActionNode::FindActionKeyInParameter(
...
bool FT4ActionNode::FindTargetObjectInParameter(
...
bool FT4ActionNode::FindTargetLocationInParameter(
...
bool FT4ActionNode::FindTargetDirectionInParameter(
...
void FT4ActionNode::SetDebugPause(bool bInPause) // #54
{
