...
/**
  * #99
 */
FT4ActionEnvironmentNode::FT4ActionEnvironmentNode(FT4ActionControl* InControl, const FT4ActionKey& InKey)
...
)
{
	...
	if (ET4PlayTarget::Player == InAction.PlayTarget || ET4PlayTarget::Default == InAction.PlayTarget) // #100
	{
		...
		if (!OwnerGameObject->IsPlayer()) // #100 : 옵션에 따라 플레이어가 아니면 플레이하지 않는다.
		{
			return nullptr;
		}
	}
...
bool FT4ActionEnvironmentNode::Create(const FT4ActionStruct* InAction)
{
	check(ET4ActionType::Environment == InAction->ActionType);
...
bool FT4ActionEnvironmentNode::Play()
{
	...
	EnvironmentZoneComponentPtr = NewComponentTemplate<UT4EnvironmentZoneComponent>(true);
	{
