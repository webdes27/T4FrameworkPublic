...
#include "Object/Component/T4ParticleSystemComponent.h" // #110
...
AT4MovableItemObject::AT4MovableItemObject(const FObjectInitializer& ObjectInitializer)
	...
	, ParticleSystemComponent(nullptr) // #110
...
void AT4MovableItemObject::Reset()
{
	...
	ParticleSystemLoader.Reset(); // #110
	...
	if (nullptr != ParticleSystemComponent) // #110
	{
...
bool AT4MovableItemObject::CreateParticleSystem() // #110
{
	check(EntityAssetPtr.IsValid());
	check(nullptr == ParticleSystemComponent);
	const FT4EntityItemDropMeshData& MeshData = EntityAssetPtr->DropMeshData;

	ParticleSystemComponent = NewObject<UT4ParticleSystemComponent>(this);
	check(nullptr != ParticleSystemComponent);
	{
...
bool AT4MovableItemObject::CheckAsyncLoading()
{
	...
	else if (ET4EntityMeshType::ParticleSystem == MeshType) // #110
	{
...
void AT4MovableItemObject::SetEntityAttributes()
{
	check(EntityAssetPtr.IsValid());
	const FT4EntityItemDropMeshData& DropMeshAttribute = EntityAssetPtr->DropMeshData;
	...
	}
	{
		if (nullptr != ActiveMeshComponent)
		{
			...
			ActiveMeshComponent->SetRelativeLocationAndRotation(
				FVector(0.0f, 0.0f, -Property.HalfHeight) + DropMeshAttribute.DropOffset,
				Property.MeshImportRotation + DropMeshAttribute.DropAngle
			);
		}
