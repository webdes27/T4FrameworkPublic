...
namespace T4EngineUtility
{
	...
	AT4MapZoneVolume* FindMapZomeVolumeOnWorld(
		UWorld* InWorld,
		FName InMapZoneName
	)
	{
		check(nullptr != InWorld);
		for (TActorIterator<AT4MapZoneVolume> It(InWorld); It; ++It)
		{
			AT4MapZoneVolume* ZoneVolume = *It;
			check(nullptr != ZoneVolume);
			if (ZoneVolume->ZoneName == InMapZoneName)
			{
				return ZoneVolume;
			}
		}
	...
	)
	{
		...
		for (TActorIterator<AT4MapZoneVolume> It(InWorld); It; ++It)
		{
			AT4MapZoneVolume* ZoneVolume = *It;
			check(nullptr != ZoneVolume);
			OutMapZoneVolumes.Add(ZoneVolume);
		}
		return (0 < OutMapZoneVolumes.Num()) ? true : false;
	}

#if WITH_EDITOR
	AT4MapZoneVolume* SpawnMapZomeVolumeOnWorld(
		UWorld* InWorld,
	...
	) // #94
	{
		...
		SpawnInfo.ObjectFlags |= (bTransient) ? RF_Transient : RF_NoFlags;
		SpawnInfo.SpawnCollisionHandlingOverride = ESpawnActorCollisionHandlingMethod::AlwaysSpawn;
		SpawnInfo.bDeferConstruction = false;
		AT4MapZoneVolume* NewMapZoneVolume = InWorld->SpawnActor<AT4MapZoneVolume>(
		...
		//NewMapZoneVolume->SetActorLocationAndRotation(InLocation, InRotation);
		NewMapZoneVolume->SetActorScale3D(InScale);

		{
			// #92 : refer CreateBrushForVolumeActor
			NewMapZoneVolume->PolyFlags = 0;
			NewMapZoneVolume->Brush = NewObject<UModel>(NewMapZoneVolume, NAME_None, RF_Transactional);
			NewMapZoneVolume->Brush->Initialize(nullptr, true);
			NewMapZoneVolume->Brush->Polys = NewObject<UPolys>(NewMapZoneVolume->Brush, NAME_None, RF_Transactional);
			NewMapZoneVolume->GetBrushComponent()->Brush = NewMapZoneVolume->Brush;

			if (ET4EntityZoneBrushType::Cube == InBrushType)
			{
				UCubeBuilder* BrushBuilder = NewObject<UCubeBuilder>();
				check(nullptr != BrushBuilder);
				NewMapZoneVolume->BrushBuilder = DuplicateObject<UBrushBuilder>(BrushBuilder, NewMapZoneVolume);
				BrushBuilder->Build(InWorld, NewMapZoneVolume);
			}
			...
			FBSPOps::csgPrepMovingBrush(NewMapZoneVolume);

			// Set the texture on all polys to NULL.  This stops invisible textures
			// dependencies from being formed on volumes.
			if (NewMapZoneVolume->Brush)
			{
				for (int32 poly = 0; poly < NewMapZoneVolume->Brush->Polys->Element.Num(); ++poly)
				{
					FPoly* Poly = &(NewMapZoneVolume->Brush->Polys->Element[poly]);
					Poly->Material = nullptr;
				}
			}
		}

		return NewMapZoneVolume;
	}
#endif

	FName GetNextTimeTagName(FName InTimeName) // #93
	{
		if (T4WorldEivronmentTimeTagNameOfDay == InTimeName)
		{
			return T4WorldEivronmentTimeTagNameOfAtNoon; // #97 : 정오, 한밤 추가!
		}
		if (T4WorldEivronmentTimeTagNameOfAtNoon == InTimeName) // #97 : 정오, 한밤 추가!
		{
			return T4WorldEivronmentTimeTagNameOfSunset;
		}
		else if (T4WorldEivronmentTimeTagNameOfSunset == InTimeName)
		{
			return T4WorldEivronmentTimeTagNameOfNight;
		}
		else if (T4WorldEivronmentTimeTagNameOfNight == InTimeName)
		{
			return T4WorldEivronmentTimeTagNameOfMidnight; // #97 : 정오, 한밤 추가!
		}
		else if (T4WorldEivronmentTimeTagNameOfMidnight == InTimeName) // #97 : 정오, 한밤 추가!
		{
			return T4WorldEivronmentTimeTagNameOfSunrise;
		}
		else if (T4WorldEivronmentTimeTagNameOfSunrise == InTimeName)
		{
			return T4WorldEivronmentTimeTagNameOfDay;
		}
		return T4WorldEivronmentTimeTagNameOfFallback;
	}
	...
	FName GetPrevTimeTagName(FName InTimeName) // #93
	{
		...
		else if (T4WorldEivronmentTimeTagNameOfAtNoon == InTimeName) // #97 : 정오, 한밤 추가!
		{
			return T4WorldEivronmentTimeTagNameOfDay;
		}
		else if (T4WorldEivronmentTimeTagNameOfSunset == InTimeName)
		...
		{
			return T4WorldEivronmentTimeTagNameOfNight; // #97 : 정오, 한밤 추가!
		}
		else if (T4WorldEivronmentTimeTagNameOfSunrise == InTimeName)
		{
			return T4WorldEivronmentTimeTagNameOfMidnight; // #97 : 정오, 한밤 추가!
	...
	bool ApplyPostProcess(
		...
		AT4MapZoneVolume* InCachedVolume // #98
	)
	{
		if (nullptr == InCachedVolume)
		{
			InCachedVolume = FindGlobalMapZoneVolume(InWorld);
		}
		if (nullptr == InCachedVolume)
		{
			return false;
	...
	AT4MapZoneVolume* FindGlobalMapZoneVolume(UWorld* InWorld)
	{
		return FindMapZomeVolumeOnWorld(InWorld, T4WorldGlobalMapZoneName);
	}
	// #97
		...
		for (TActorIterator<ADirectionalLight> It(InWorld); It; ++It)
		{
			...
			if (bHasData)
			{
				UE_LOG(
					LogT4Engine,
					Warning,
					TEXT("GetDirectionalData : already exists. skipped")
				);
				break;
			}
		...
		{	
			...
			if (bHasData)
			{
				UE_LOG(
					LogT4Engine,
					Warning,
					TEXT("GetDirectionalLightData : already exists. skipped")
				);
				break;
			}
			UDirectionalLightComponent* Component = Cast<UDirectionalLightComponent>(Actor->GetLightComponent());
		...
		for (TActorIterator<AActor> It(InWorld, BPSkySphereClass); It; ++It)
		{
			...
			if (bHasData)
			{
				UE_LOG(
					LogT4Engine,
					Warning,
					TEXT("GetBPSkySphereData : already exists. skipped")
				);
				break;
			}
			{
		...
		for (TActorIterator<ASkyLight> It(InWorld); It; ++It)
		{
			...
			if (bHasData)
			{
				UE_LOG(
					LogT4Engine,
					Warning,
					TEXT("GetSkyLightData : already exists. skipped")
				);
				break;
			}
		...
		for (TActorIterator<AAtmosphericFog> It(InWorld); It; ++It)
		{
			...
			if (bHasData)
			{
				UE_LOG(
					LogT4Engine,
					Warning,
					TEXT("GetAtmosphericFogData : already exists. skipped")
				);
				break;
			}
		...
		for (TActorIterator<AExponentialHeightFog> It(InWorld); It; ++It)
		{
			...
			if (bHasData)
			{
				UE_LOG(
					LogT4Engine,
					Warning,
					TEXT("GetExponentialHeightFogData : already exists. skipped")
				);
				break;
			}
