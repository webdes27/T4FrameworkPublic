...
bool FT4MaterialLoader::Process(UDecalComponent* InDecalComponent)
{
	...
	if (nullptr == MaterialInterface)
	{
		T4_LOG(
			Error,
			TEXT("Failed to convert UMaterialInterface. (%s, %s)"),
			*(DebugToken.ToString()),
...
UMaterialInterface* FT4MaterialLoader::GetMaterialInterface() // #56
{
	...
	if (nullptr == LoadedObject)
	{
		T4_LOG(
			Error,
			TEXT("Failed to load Material. (%s, %s)"),
			*(DebugToken.ToString()),
			*(LoadHandle->GetObjectPath().ToString())
		);
		return nullptr;
...
bool FT4AnimBlueprintClassLoader::Process(USkeletalMeshComponent* InMeshComponent)
{
	...
	if (nullptr == LoadedBPObject)
	{
		T4_LOG(
			Error,
			TEXT("Failed to load Anim Blueprint. (%s, %s)"),
			*(DebugToken.ToString()),
			*(LoadHandle->GetObjectPath().ToString())
		);
		return false;
	...
	if (nullptr == AnimBPClass)
	{
		T4_LOG(
			Error,
			TEXT("Convert to AnimBlueprint (%s, %s) failed."),
			*(DebugToken.ToString()),
			*(LoadHandle->GetObjectPath().ToString())
		);
		return false;
...
bool FT4PhysicsAssetLoader::Process(USkeletalMeshComponent* InMeshComponent) // #76
{
	...
	if (nullptr == LoadedObject)
	{
		T4_LOG(
			Error,
			TEXT("Failed to load Physics asset. (%s, %s)"),
			*(DebugToken.ToString()),
			*(LoadHandle->GetObjectPath().ToString())
		);
		return false;
	...
	if (nullptr == PhysicsAsset)
	{
		T4_LOG(
			Error,
			TEXT("Failed to convert UPhysicsAsset. (%s, %s)"),
			*(DebugToken.ToString()),
			*(LoadHandle->GetObjectPath().ToString())
		);
		return false;
...
bool FT4StaticMeshLoader::Process(UStaticMeshComponent* InMeshComponent)
{
	...
	if (nullptr == LoadedObject)
	{
		T4_LOG(
			Error,
			TEXT("Failed to load Static mesh. (%s, %s)"),
			*(DebugToken.ToString()),
			*(LoadHandle->GetObjectPath().ToString())
		);
		return false;
	...
	if (nullptr == StaticMesh)
	{
		T4_LOG(
			Error,
			TEXT("Failed to convert UStaticMesh. (%s, %s)"),
			*(DebugToken.ToString()),
			*(LoadHandle->GetObjectPath().ToString())
		);
		return false;
...
bool FT4SkeletalMeshLoader::Process(USkinnedMeshComponent* InMeshComponent)
{
	...
	if (nullptr == LoadedObject)
	{
		T4_LOG(
			Error,
			TEXT("Failed to load Skeletal mesh. (%s, %s)"),
			*(DebugToken.ToString()),
			*(LoadHandle->GetObjectPath().ToString())
		);
		return false;
	...
	if (nullptr == SkeletalMesh)
	{
		T4_LOG(
			Error,
			TEXT("Failed to convert USkeletalMesh. (%s, %s)"),
			*(DebugToken.ToString()),
			*(LoadHandle->GetObjectPath().ToString())
		);
		return false;
...
bool FT4ParticleSystemLoader::Process(UParticleSystemComponent* InParticleSystemComponent)
{
	...
	if (nullptr == ParticleSystem)
	{
		T4_LOG(
			Error,
			TEXT("Failed to convert UParticleSystem. (%s, %s)"),
			*(DebugToken.ToString()),
			*(LoadHandle->GetObjectPath().ToString())
		);
		return false;
...
UParticleSystem* FT4ParticleSystemLoader::GetParticleSystem() // #56
{
	...
	if (nullptr == LoadedObject)
	{
		T4_LOG(
			Error,
			TEXT("Failed to load ParticleSystem. (%s, %s)"),
			*(DebugToken.ToString()),
			*(LoadHandle->GetObjectPath().ToString())
		);
		return nullptr;
