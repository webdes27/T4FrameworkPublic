...
#include "Public/T4EngineEnvironment.h" // #93
...
void FT4WorldTimeControl::StartPlay(float InStartHour)
{
	ConstantGameTimeHoursPerDay = T4EngineConstant::GetGameTimeHoursPerDay();
	...
	{
		AddConstantGameTimeData(T4Const_WorldTimeTagDayName, T4Const_WorldTimeTagNoonName);
		AddConstantGameTimeData(T4Const_WorldTimeTagNoonName, T4Const_WorldTimeTagSunsetName); // #97 : 정오, 한밤 추가!
		AddConstantGameTimeData(T4Const_WorldTimeTagSunsetName, T4Const_WorldTimeTagNightName);
		AddConstantGameTimeData(T4Const_WorldTimeTagNightName, T4Const_WorldTimeTagMidnightName); // #97 : 정오, 한밤 추가!
		AddConstantGameTimeData(T4Const_WorldTimeTagMidnightName, T4Const_WorldTimeTagSunriseName);
		AddConstantGameTimeData(T4Const_WorldTimeTagSunriseName, T4Const_WorldTimeTagDayName);
	}
...
FName FT4WorldTimeControl::GetPrevTimeTagName() const
{
	...
	return T4EngineEnvironment::GetPrevTimeTagName(CurrentTimeTagName);
}

FName FT4WorldTimeControl::GetNextTimeTagName() const
{
	FName CurrentTimeTagName = GetTimeTagName();
	return T4EngineEnvironment::GetNextTimeTagName(CurrentTimeTagName);
}
...
) // #93
{
	...
	const FT4ConstantDataRow& StartTimeDataRow = ConstantTableManager->GetConstantData(
		ET4EngineConstantTable::TimeTag,
		InStartTimeName
	);
	const FT4ConstantDataRow& EndTimeDataRow = ConstantTableManager->GetConstantData(
		ET4EngineConstantTable::TimeTag,
		InEndTimeName
	);
