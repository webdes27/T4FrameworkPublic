...
void FT4WorldContainer::ProcessPre(float InDeltaTime) // #34 : OnWorldPreActorTick
{
	if (0 < PendingRemoveClientObjectIDs.Num())
	{
		...
		{
			RemoveGameObject(ClientObjectID, T4Const_ObjectWorldLeaveTimeSec);
		}
		PendingRemoveClientObjectIDs.Empty();
	}
...
)
{
	...
	else if (SpawnedGameObjectMap.Contains(InAction.ObjectID))
	{
		T4_LOG(
			Error,
			TEXT("Already spawned. ObjectID = '%u'"),
			*InAction.ObjectID
		);
		return false;
	}
...
)
{
	...
	else if (ET4SpawnMode::Client == InObjectID.SpawnMode)
	{
		T4_LOG(
			Error,
			TEXT("Incorrect Object Spawn Type")
		);
		ensure(false);
		return false;
	}
...
bool FT4WorldContainer::GetStaticWorldZoneVolumes(
	TArray<AT4WorldZoneVolume*>& OutZoneVolumes
) // #94
{
	check(nullptr != GameWorldRef);
	UWorld* UnrealWorld = GameWorldRef->GetWorld();
	...
	return T4EngineEnvironment::GetWorldZoneVolumesOnWorld(UnrealWorld, OutZoneVolumes);
}
...
)
{
	...
	if (PendingRemoveClientObjectIDs.Contains(InObjectID))
	{
		T4_LOG(
			Error,
			TEXT("Already Pending ClientObject. ObjectID = '%s'"),
			*(InObjectID.ToString())
		);
		return false;
	}
...
bool FT4WorldContainer::AddGameObject(AT4GameObject* InNewObject) // #54
{
	...
	if (SpawnedGameObjectMap.Contains(ObjectID))
	{
		T4_LOG(
			Error,
			TEXT("Already spawned. ObjectID = '%s'"),
			*(ObjectID.ToString())
		);
		return false;
	}
...
) // #54
{
	if (!SpawnedGameObjectMap.Contains(InObjectID))
	{
		T4_LOG(
			Error,
			TEXT("Spawn Object not found. ObjectID = '%s'"),
			*(InObjectID.ToString())
		);
		return false;
	}
