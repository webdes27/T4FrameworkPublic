...
#include "Classes/World/T4MapZoneVolume.h" // #94
...
void FT4WorldContextPreviewScene::Reset()
{
	...
	if (GlobalMapZoneVolumePtr.IsValid()) // #98
	{
		GlobalMapZoneVolumePtr->RemoveFromRoot();
...
void FT4WorldContextPreviewScene::CreateEnvironment() // #94
{
	...
	// #T4_ADD_TOD_TAG
	{
		DirectionalLightActorPtr = T4EngineUtility::SpawnDirectionalLightActor(PreviewWorld, RF_Transient);
		check(DirectionalLightActorPtr.IsValid());
		DirectionalLightActorPtr->AddToRoot();
	}
	{
		BPSkySphereActorPtr = T4EngineUtility::SpawnBPSkySphereActor(PreviewWorld, RF_Transient);
		check(BPSkySphereActorPtr.IsValid());
		BPSkySphereActorPtr->AddToRoot();
	}
	{
		SkyLightActorPtr = T4EngineUtility::SpawnSkyLightActor(PreviewWorld, RF_Transient);
		check(SkyLightActorPtr.IsValid());
		SkyLightActorPtr->AddToRoot();
	}
	{
		AtmosphericFogActorPtr = T4EngineUtility::SpawnAtmosphericFogActor(PreviewWorld, RF_Transient);
		check(AtmosphericFogActorPtr.IsValid());
		AtmosphericFogActorPtr->AddToRoot();
	}
	{
		ExponentialHeightFogActorPtr = T4EngineUtility::SpawnExponentialHeightFogActor(PreviewWorld, RF_Transient);
		check(ExponentialHeightFogActorPtr.IsValid());
		ExponentialHeightFogActorPtr->AddToRoot();
	}
	{
		GlobalMapZoneVolumePtr = T4EngineUtility::SpawnMapZomeVolumeOnWorld(
			PreviewWorld,
			FVector::ZeroVector,
		...
		check(GlobalMapZoneVolumePtr.IsValid());
		GlobalMapZoneVolumePtr->AddToRoot();
		UT4EngineSettings* EngineSettings = GetMutableDefault<UT4EngineSettings>();
		check(nullptr != EngineSettings);
		if (bThumbnailMode)
		{
			GlobalMapZoneVolumePtr->EnvironmentAsset = EngineSettings->ThumbnailSceneEnvironmentAsset; // #97
		}
		else
		{
			GlobalMapZoneVolumePtr->EnvironmentAsset = EngineSettings->PreviewSceneEnvironmentAsset;
		}
		GlobalMapZoneVolumePtr->ZoneName = T4WorldGlobalMapZoneName;
		GlobalMapZoneVolumePtr->BlendPriority = -1;
		GlobalMapZoneVolumePtr->BlendInTimeSec = 1.0f;
		GlobalMapZoneVolumePtr->BlendOutTimeSec = 1.0f;
		GlobalMapZoneVolumePtr->DebugColor = FColor::White;
		if (nullptr != GlobalMapZoneVolumePtr->GetBrushComponent())
		{
			GlobalMapZoneVolumePtr->GetBrushComponent()->SetMobility(EComponentMobility::Movable);
		}
		GlobalMapZoneVolumePtr->Modify();
	}
}
