...
#include "ST4EquipPointDropListWidget.h"
...
#define LOCTEXT_NAMESPACE "ST4EquipPointDropListWidget"

/**
  * #74
 */

ST4EquipPointDropListWidget::ST4EquipPointDropListWidget()
{
}
...
void ST4EquipPointDropListWidget::Construct(
...
)
{
	...
	if (ViewTargetSelector.IsValid())
	{
		ViewTargetSelectDelegate = ViewTargetSelector.Pin()->GetOnViewTargetChanged().AddRaw(
			this,
			&ST4EquipPointDropListWidget::HandleOnViewTargetChanged
		);
	}
...
void ST4EquipPointDropListWidget::HandleOnViewTargetChanged(IT4GameObject* InViewTarget)
{
	Refresh();
}

void ST4EquipPointDropListWidget::UpdateItemLists()
{
	...
	TArray<FT4ConstantDataRow>& ConstantDatas = EngineConstants->GetConstantDatas(ET4EngineConstantType::EquipPoint);
