...
#include "T4RehearsalViewportViewMenu.h"
...
#define LOCTEXT_NAMESPACE "RehearsalViewportViewMenu"

void ST4RehearsalViewportViewMenu::Construct( const FArguments& InArgs, ST4RehearsalViewport* InViewport, TSharedRef<class SViewportToolBar> InParentToolBar )
{
	...
	(
		SEditorViewportToolbarMenu::FArguments()
			...
			.Label(this, &ST4RehearsalViewportViewMenu::GetViewMenuLabel)
			.LabelIcon(this, &ST4RehearsalViewportViewMenu::GetViewMenuLabelIcon)
			.OnGetMenuContent( this, &ST4RehearsalViewportViewMenu::GenerateViewMenuContent )
	);
}

FText ST4RehearsalViewportViewMenu::GetViewMenuLabel() const
{
	FText Label = LOCTEXT("ViewMenuTitle_Default", "View");
	if (nullptr != ViewportRef)
	{
		switch(ViewportRef->GetViewportClient()->GetViewMode() )
		{
...
const FSlateBrush* ST4RehearsalViewportViewMenu::GetViewMenuLabelIcon() const
{
...
TSharedRef<SWidget> ST4RehearsalViewportViewMenu::GenerateViewMenuContent() const
{
	const FEditorViewportCommands& BaseViewportActions = FEditorViewportCommands::Get();

	const bool bInShouldCloseWindowAfterMenuSelection = true;
	FMenuBuilder ViewMenuBuilder(bInShouldCloseWindowAfterMenuSelection, ViewportRef->GetCommandList(), MenuExtenders);

	{
		...
		// Auto Exposure
		{
			...
			TSharedRef<SWidget> FixedEV100Menu = ViewportRef->BuildFixedEV100Menu();

			ViewMenuBuilder.BeginSection("Exposure", LOCTEXT("ExposureHeader", "Exposure"));
			ViewMenuBuilder.AddMenuEntry( BaseViewportCommands.ToggleAutoExposure, NAME_None );
