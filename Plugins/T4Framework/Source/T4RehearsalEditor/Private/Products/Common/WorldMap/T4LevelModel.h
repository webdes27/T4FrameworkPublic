...
typedef TArray<TSharedPtr<class FT4LevelModel>> FT4LevelModelList;
class FT4LevelCollectionModel;

/**
 *  Interface for level collection hierarchy traversal
 */
struct FT4LevelModelVisitor
{
	virtual ~FT4LevelModelVisitor() { }
	virtual void Visit(FT4LevelModel& Item) = 0;
};
 ...
 */
class FT4LevelModel
	: public TSharedFromThis<FT4LevelModel>	
{
public:
	typedef IFilter<const TWeakObjectPtr<AActor>&> ActorFilter;

	DECLARE_EVENT( FT4LevelModel, FSimpleEvent );
	
public:
	FT4LevelModel(FT4LevelCollectionModel& InLevelCollectionModel);

	virtual ~FT4LevelModel();
	
	/** Traverses level model hierarchy */
	void Accept(FT4LevelModelVisitor& Vistor);
	 ...
	 */
	bool AttachTo(TSharedPtr<FT4LevelModel> InParent);

	/**	Notifies level model that filters has been changed */
	void OnFilterChanged();

	/**	@return Level child hierarchy */
	const FT4LevelModelList& GetChildren() const;
	
	/**	@return Parent level model */
	TSharedPtr<FT4LevelModel> GetParent() const;
	
	/**	Sets link to a parent model  */
	void SetParent(TSharedPtr<FT4LevelModel>);

	/**	Removes all entries from children list*/
	void RemoveAllChildren();

	/**	Removes specific child */
	void RemoveChild(TSharedPtr<FT4LevelModel> InChild);
	
	/**	Adds new entry to a children list */
	void AddChild(TSharedPtr<FT4LevelModel> InChild);
		
	/**	@return Whether this model has in ancestors specified level model */
	bool HasAncestor(const TSharedPtr<FT4LevelModel>& InLevel) const;

	/**	@return Whether this model has in descendants specified level model */
	bool HasDescendant(const TSharedPtr<FT4LevelModel>& InLevel) const;
...
protected:
	...
	FT4LevelCollectionModel&			LevelCollectionModel;
		
	/** The parent level  */
	TWeakPtr<FT4LevelModel>				Parent;
		
	/** Filtered children of this level  */
	FT4LevelModelList					FilteredChildren;

	/** All children of this level  */
	FT4LevelModelList					AllChildren;
