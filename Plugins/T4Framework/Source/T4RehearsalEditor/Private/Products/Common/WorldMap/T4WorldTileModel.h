...
class UT4WorldTileDetails;

//
typedef TArray<TSharedPtr<class FT4WorldTileModel>> FT4WorldTileModelList;
 ...
 */
class FT4WorldTileModel 
	: public FT4LevelModel	
{
public:
	struct FCompareByLongPackageName
	{
		FORCEINLINE bool operator()(const TSharedPtr<FT4LevelModel>& A, 
									const TSharedPtr<FT4LevelModel>& B) const
		{
	...
	/**
	 *	FT4WorldTileModel Constructor
	 *
	 *	@param	InWorldData		WorldBrowser world data
	 *	@param	TileIdx			Tile index in world composition tiles list
	 */
	FT4WorldTileModel(FT4WorldTileCollectionModel& InWorldData, int32 TileIdx);
	~FT4WorldTileModel();

public:
	// FT4LevelModel interface
	...
	/**  */
	bool CreateAdjacentLandscapeProxy(ALandscapeProxy* SourceLandscape, FT4WorldTileModel::EWorldDirections InWhere);
...
public:
	...
	UT4WorldTileDetails*					TileDetails;
