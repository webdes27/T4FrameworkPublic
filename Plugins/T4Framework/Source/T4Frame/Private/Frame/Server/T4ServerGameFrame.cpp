...
void FT4ServerGameFrame::ResetPre()
{
	for (TMap<FT4NetID, IT4GameAIController*>::TConstIterator It = GameAIControllerRefs.CreateConstIterator(); It; ++It)
	{
		...
		//       참고로 여기는 강제 종료시 호출됨. 현재 T4Gameplay 에서는 따로 컨테이너가 없기 때문. 좋은 방법이 있으면 변경!
		// #104 : T4Gameplay 모듈에서 Spawn 시켜놓고 소멸 되지 않도록 AddToRoot 를 했는데,
...
)
{
	...
	if (GameAIControllerRefs.Contains(InUniqueID))
	{
		UE_LOG(
			LogT4Frame,
			Error,
			TEXT("FT4ServerGameFrame : already added. %s"),
			*(InUniqueID.ToString())
		);
		return false;
	}
...
void FT4ServerGameFrame::UnregisterGameAIController(const FT4NetID& InUniqueID)
{
	...
	if (!GameAIControllerRefs.Contains(InUniqueID))
	{
		UE_LOG(
			LogT4Frame,
			Error,
			TEXT("FT4ServerGameFrame : added contgroller not found. UniqueID = '%u'"),
			*(InUniqueID.ToString())
		);
		return;
	}
