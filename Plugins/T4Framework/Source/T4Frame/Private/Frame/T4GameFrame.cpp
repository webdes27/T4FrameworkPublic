...
#include "T4Engine/Public/Action/T4ActionWorldCommands.h" // #87
...
)
{
	...
	if (!bInitialized)
	{
		T4_LOG(
			Display,
			TEXT("[SL:%u] Failed to Initialized"),
			uint32(LayerType)
		);
...
bool FT4GameFrame::OnWorldTravel(const UT4MapEntityAsset* InMapEntityAsset) // #87
{
	...
	// OnReset(); // #87 : 월드 이동 ActionReplay 지원 : FT4EngineDelegates::OnGameWorldTravelPre : 
	{
...
#endif
		check(nullptr != GameWorld);
		GameWorld->OnExecuteAction(&NewAction); // #83, #87
	}
	// OnStartPlay(); // // #87 : 월드 이동 ActionReplay 지원 : FT4EngineDelegates::OnGameWorldTravelPost
	return true;
}

void FT4GameFrame::HandleOnGameWorldTravelPre(IT4GameWorld* InGameWorld) // #87 : 월드 이동 ActionReplay 지원
{
...
void FT4GameFrame::HandleOnGameWorldTravelPost(IT4GameWorld* InGameWorld) // #87 : 월드 이동 ActionReplay 지원
{
	if (nullptr == InGameWorld || GameWorld != InGameWorld)
	{
		return;
