...
AT4PlayerController::AT4PlayerController(const FObjectInitializer& ObjectInitializer)
	...
	, bMouseCursorLocked(false)
	, SaveMousePosition(FVector2D::ZeroVector)
...
#endif
{
	...
	//        bShowMouseCursor flag 를 끄고, bMouseCursorLocked flag 로 MouseCursor 를 사용하도록 제어 
...
void AT4PlayerController::EndPlay(const EEndPlayReason::Type InEndPlayReason)
{
	...
	if (!T4EngineLayer::IsServer(LayerType))
	{
		...
		GameWorld->SetClientNetworkControl(nullptr);
	}
...
void AT4PlayerController::BeginPlay()
{
	...
	if (!T4EngineLayer::IsServer(LayerType))
	{
		...
		IT4GameWorld* GameWorld = T4EngineWorldGet(LayerType);
		check(nullptr != GameWorld);
		GameWorld->SetClientNetworkControl(static_cast<IT4NetworkControl*>(this));
...
void AT4PlayerController::OnSetInputMode(ET4InputMode InMode)
{
	...
	if (GameAndUI == InMode)
	{
		...
		InputMode.SetHideCursorDuringCapture(false); // #113 : Mouse Left Key Pressed 후에도 마우스 위치 업데이트를 위한 조치 (Aiming Attack)
		InputMode.SetLockMouseToViewportBehavior(EMouseLockMode::LockOnCapture); // #116 : DefaultInput (DefaultViewportMouseLockMode=LockOnCapture)
...
bool AT4PlayerController::HasAction(const FT4ActionKey& InActionKey) const
{
	// #20
...
bool AT4PlayerController::IsPlayingAction(const FT4ActionKey& InActionKey) const
{
	// #20
	IT4GameObject* ViewTargetObject = GetGameObject();
	if (nullptr == ViewTargetObject)
...
)
{
	...
	if (T4EngineLayer::IsToolSide(LayerType))
	{
		// #30
		check(nullptr != EditorViewportClient);
		bool bResult = EditorViewportClient->GetMousePositionToWorldRay(OutStartPosition, OutStartDirection);
		return bResult;
	}
#endif
	bool bResult = DeprojectMousePositionToWorld(OutStartPosition, OutStartDirection);
	return bResult;
}

void AT4PlayerController::SetMouseCursorLock(bool bInLock)
{
	if (T4EngineLayer::IsServer(LayerType))
	{
		ensure(false);
	...
	bMouseCursorLocked = bInLock;
	if (bInLock)
	{
		GetMouseCursorPosition(SaveMousePosition);
	}
	else
	{
		// #116 : 커서가 안보이다 보이면 저장했던 시작 위치로 보내준다.
		SetMouseCursorPosition(SaveMousePosition);
		SaveMousePosition = FVector2D::ZeroVector;
	}
}

EMouseCursor::Type AT4PlayerController::GetMouseCursor() const
{
	// #116 : 마우스 우클릭으로 카메라 조정시 bShowMouseCursor flag 를 켜주면 자연스러운 움직임을 캡처할 수가 없어
	//        bShowMouseCursor flag 를 끄고, bMouseCursorLocked flag 로 MouseCursor 를 사용하도록 제어 (Only PIE)
	if (!bMouseCursorLocked) // ShouldShowMouseCursor()
	{
...
void AT4PlayerController::SetMouseCursorType(EMouseCursor::Type InCursorType)
{
	CurrentMouseCursor = InCursorType;
	// #30
#if WITH_EDITOR
	if (T4EngineLayer::IsToolSide(LayerType))
	{
		check(nullptr != EditorViewportClient);
		EditorViewportClient->SetMouseCursorType(InCursorType);
	}
#endif
}
...
void AT4PlayerController::SetMouseCursorPosition(const FVector2D& InPosition) // #30, #113
{
#if WITH_EDITOR
	if (T4EngineLayer::IsToolSide(LayerType))
	{
		check(nullptr != EditorViewportClient);
		EditorViewportClient->SetMouseLocation(InPosition.X, InPosition.Y);
		return;
	}
#endif
	SetMouseLocation(InPosition.X, InPosition.Y);
}

bool AT4PlayerController::GetMouseCursorPosition(FVector2D& OutPosition) const // #30, #113
{
	// #30
#if WITH_EDITOR
	if (T4EngineLayer::IsToolSide(LayerType))
	{
		check(nullptr != EditorViewportClient);
		return EditorViewportClient->GetMousePosition(OutPosition.X, OutPosition.Y);
	}
#endif
	return GetMousePosition(OutPosition.X, OutPosition.Y);
}
		...
		default:
			{
				T4_LOG(
					Error,
					TEXT("[SL:%u] No implementation Type '%u'"),
					uint32(LayerType),
